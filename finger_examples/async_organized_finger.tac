...
def main(): # quitamos la construcción de los factories aquí y los centralizamos en un service
    ...
    f = async_finger.FingerService("/etc/users")
    finger = strports.service("tcp:79", async_finger.IFingerFactory(f)) # no te confundas esto es un servicio como el de arriba
    ...
    i = async_finger.IIRCClientFactory(f)
    ...
    strports.service(
        "tcp:8889", pb.PBServerFactory(async_finger.IPerspectiveFinger(f))
